...
# Function to test RHSM registration status
test_rhsm() {
    local TARGET_HOST="$1"
    local TEST_GROUP="Subscription"
    local TEST_NAME="Check registration status.."

    # Connect via SSH and attempt to get the system identity
    if ssh -o BatchMode=yes -o ConnectTimeout=5 -o StrictHostKeyChecking=no "$TARGET_HOST" "sudo subscription-manager identity" >/dev/null 2>&1; then
        # Format: [Result] | [Group] | [Test] | [Target]
        # We manually pad "OK" with spaces to 5 characters (" OK  ") to center it
